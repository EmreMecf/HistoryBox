    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        tooltip: _story!.isFavorite
            ? AppLocalizations.of(context)?.removeFromFavorites ?? 'Favorilerden Çıkar'
            : AppLocalizations.of(context)?.addToFavorites ?? 'Favorilere Ekle',
        onPressed: () async {
          final previousState = _story!.isFavorite;
          ...
          if (result is Failure && mounted) {
            ...
            setState(() {
              _story = _story!.copyWith(isFavorite: previousState);
            });

            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  AppLocalizations.of(context)?.error_occurred ?? 'Bir hata oluştu',
                ),
              ),
            );
          }
