...
class _StoryDetailScreenState extends State<StoryDetailScreen> {
  ...
  final AdService _adService = injector<AdService>();
  StoryModel? _story;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        tooltip: _story!.isFavorite ? 'Favorilerden Çıkar' : 'Favorilere Ekle',
        onPressed: () async {
          final newStatus = !_story!.isFavorite;
          final previousStory = _story;

          // Optimistic update
          setState(() {
            _story = _story!.copyWith(isFavorite: newStatus);
          });
          ...
          if (result is Success<void, Exception>) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  newStatus ? 'Favorilere eklendi' : 'Favorilerden çıkarıldı',
                ),
            ...
            // Revert on failure
            setState(() {
              _story = previousStory;
            });
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: const Text('İşlem başarısız oldu'),
        ...
        child: Icon(
          ...
          semanticLabel: _story!.isFavorite ? 'Favorilerden Çıkar' : 'Favorilere Ekle',
        ),
      ),
    );
  }
