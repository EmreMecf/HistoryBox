...
import '../../services/apis/gemini_api_client.dart';
...
class AiStoryService {
  ...
  Future<Result<Map<String, String>, Exception>> generateStory({
    ...
    try {
      ...
      // Gemini'den yanıt al
      final content = await _ai.generateText(prompt);
  ...
  // Çizimden (görselden) masal üret
  Future<Result<Map<String, String>, Exception>> generateStoryFromImage({
    ...
    try {
      AppLogger.log('Generating story from drawing', tag: 'AiStoryService');

      final ageDesc = _getAgeGroupDescription(ageGroup);
      final hero = (childName != null && childName.trim().isNotEmpty)
          ? 'Ana karakter "${childName.trim()}" adında bir çocuk olsun.\n'
          : '';

      final prompt =
          '''
Bu bir çocuğun çizimi. Çizimde gördüklerinden ilham alarak çocuklar için
eğlenceli ve eğitici bir masal yaz.
$hero
Yaş grubu: $ageDesc

Kurallar:
1. Başlığı ilk satıra "Başlık: " ön ekiyle yaz (bu ön eki AYNEN koru, çevirme)
2. Tüm masalı $language dilinde yaz
3. Yaklaşık 400 kelime, pozitif bir mesaj
4. Şiddet, korku veya yetişkin içerik OLMASIN
5. Sadece başlık ve içeriği yaz, ek açıklama yapma.
''';

      final content = await _ai.generateTextWithImage(prompt, base64Image);

      if (content.trim().isEmpty) {
        return Failure(Exception('AI yanıt vermedi'));
      }
      AppLogger.success('Story generated from drawing', tag: 'AiStoryService');
      return Success(_parseStoryContent(content));
    } catch (e, st) {
      AppLogger.error('Error generating from drawing',
          tag: 'AiStoryService', error: e, stackTrace: st);
      return Failure(Exception('Çizimden masal oluşturulamadı: $e'));
    }
  }
  ...
  String _buildPrompt({
    ...
    final hero = (childName != null && childName.trim().isNotEmpty)
        ? 'Ana karakter "${childName.trim()}" adında bir çocuk olsun ve '
            'hikayenin kahramanı olsun.'
        : '';
    final interestLine = (interests != null && interests.trim().isNotEmpty)
        ? 'Çocuğun ilgi alanları: ${interests.trim()}. Bunları hikayeye doğal '
            'şekilde dahil et.'
        : '';

    return '''
$categoryInstruction
${hero.isNotEmpty ? '\n$hero' : ''}${interestLine.isNotEmpty ? '\n$interestLine' : ''}
...
Kurallar:
1. Başlığı ilk satıra "Başlık: " ile başlayarak yaz (bu "Başlık:" ön ekini AYNEN koru, çevirme)
2. İçerik başlıktan sonra gelsin
3. Çocuklar için eğitici ve eğlenceli olmalı
4. Dil basit ve anlaşılır olmalı
5. Tüm hikayeyi (başlık hariç ön ek) $language dilinde yaz
6. Pozitif bir mesaj içermeli
7. Şiddet, korku veya yetişkin içerik OLMASIN
...
''';
      ...
      final continuation = (await _ai.generateText(prompt)).trim();
  ...
  // Hikaye önerisi
  Future<Result<List<String>, Exception>> suggestTopics(
    String category, {
    String language = 'Türkçe',
  }) async {
    try {
...
Önerileri $language dilinde yaz.
