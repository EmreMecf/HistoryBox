...
class AdService {
  ...
  bool _premium = false; // premium kullanıcılarda banner/interstitial gizlenir
  RewardedAd? _cachedRewardedAd;
  InterstitialAd? _cachedInterstitialAd;
  BannerAd? _cachedBannerAd;
  bool _isBannerLoaded = false;

  /// Premium durumunu ayarlar. Premium'da rahatsız edici reklamlar (banner +
  /// interstitial) kapatılır; ödüllü reklam (token kazan) çalışmaya devam eder.
  ...
  void _preloadInterstitialAd() {
    if (!_adsEnabled || _premium) return;
    InterstitialAd.load(
  ...
  Future<Widget?> loadBannerAd() async {
    if (!_adsEnabled || _premium) return null;
    if (!_initialized) {
      await initialize();
    }
  ...
  // ==================== AD UNIT ID'LERİ ====================
  // Debug derlemelerde Google'ın resmi TEST birimleri kullanılır.
  // Release'de gerçek AdMob ID'leriniz kullanılır.
  //
  // TODO(admob): Aşağıdaki _prod* ID'lerini AdMob panelinizdeki gerçek
  // birim ID'leriyle değiştirin. Aksi halde yayında reklam GÖSTERİLMEZ
  // (geçersiz ID) ve gelir elde edilmez.

  // --- Test ID'leri (Google resmi) ---
  ...
  // --- Gerçek (production) ID'leri — DOLDURULMALI ---
  static const String _prodBannerAndroid = 'ca-app-pub-3940256099942544/6300978111';
  static const String _prodBannerIos = 'ca-app-pub-3940256099942544/2934735716';
  static const String _prodInterstitialAndroid = 'ca-app-pub-3940256099942544/1033173712';
  static const String _prodInterstitialIos = 'ca-app-pub-3940256099942544/4411468910';
  static const String _prodRewardedAndroid = 'ca-app-pub-3940256099942544/5224354917';
  static const String _prodRewardedIos = 'ca-app-pub-3940256099942544/1712485313';

  bool get _useTestAds => !kReleaseMode;

  String _getAdMobBannerId() {
    if (defaultTargetPlatform == TargetPlatform.android) {
      return _useTestAds ? _testBannerAndroid : _prodBannerAndroid;
    } else if (defaultTargetPlatform == TargetPlatform.iOS) {
      return _useTestAds ? _testBannerIos : _prodBannerIos;
    }
    return '';
  }
  ...
  String _getAdMobInterstitialId() {
    if (defaultTargetPlatform == TargetPlatform.android) {
      return _useTestAds ? _testInterstitialAndroid : _prodInterstitialAndroid;
    } else if (defaultTargetPlatform == TargetPlatform.iOS) {
      return _useTestAds ? _testInterstitialIos : _prodInterstitialIos;
    }
    return '';
  }
  ...
  String _getAdMobRewardedId() {
    if (defaultTargetPlatform == TargetPlatform.android) {
      return _useTestAds ? _testRewardedAndroid : _prodRewardedAndroid;
    } else if (defaultTargetPlatform == TargetPlatform.iOS) {
      return _useTestAds ? _testRewardedIos : _prodRewardedIos;
    }
    return '';
  }
}
